...
co2_trend <- lm(air.short$CO~air.short$year)
...
co2_comp_random <- data.frame(Date=time(co2_comp$random), Random=co2_comp$random)
NOx_comp_random <- data.frame(Date=time(NOx_comp$random), Random=NOx_comp$random)

window <- list(start=2010,end=2021)

temp_comp_random %<>% filter(Date>=window$start&Date<window$end)
co2_comp_random %<>% filter(Date>=window$start&Date<window$end)
...
co2_comp_random_ts <- ts(co2_comp_random$Random, start=c(window$start,1), frequency=freq_daily)
NOx_comp_random_ts <- ts(NOx_comp_random$Random, start=c(window$start,1), frequency=freq_daily)


# Cross correlation function
# ccf(temp_comp_random_ts,co2_comp_random_ts)
ccf(temp_comp_random_ts,co2_comp_random_ts, na.action = na.pass)
